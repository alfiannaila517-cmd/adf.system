...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_achievement') {
        ...
        if ($id > 0) {
            ...
            $row->execute([$id]); $qty = max(0.0001, (float)($row->fetchColumn() ?: 1));
            $qtyDone = min($qtyDone, $qty);
            $pct = min(100, (int)round($qtyDone / $qty * 100));
            $newStatus = ($qtyDone >= $qty) ? 'ready_ship' : 'on_progress';
            ...
            if ($newStatus === 'ready_ship') $msg .= ' Order is now Ready to Ship!';
        }
    }
}
...
$activeOrders = $pdo->query("
    ...
    ORDER BY FIELD(o.status,'on_progress','ready_ship','qc','draft','shipped'), o.id DESC
")->fetchAll();
...
$readyToShip   = count(array_filter($activeOrders, fn($r) => $r['status'] === 'ready_ship'));
...
<div class="stat-cards" style="margin-bottom:20px">
  <div class="stat-card">
    ...
    <div><div class="stat-val"><?= $totalActive ?></div><div class="stat-lbl">Active Orders</div></div>
  </div>
  <div class="stat-card">
    <div class="stat-icon" style="background:#F5F3FF"><i class="bi bi-hammer" style="color:#6D28D9"></i></div>
    <div><div class="stat-val"><?= $inProgress ?></div><div class="stat-lbl">In Production</div></div>
  </div>
  <div class="stat-card">
    <div class="stat-icon" style="background:#F0FDF4"><i class="bi bi-box-seam" style="color:#15803D"></i></div>
    <div><div class="stat-val"><?= $readyToShip ?></div><div class="stat-lbl">Ready to Ship</div></div>
  </div>
  <div class="stat-card">
...
<div class="pwf-card" style="margin-bottom:20px">
  ...
  <table class="pwf-table">
    ...
    <tbody>
      ...
      <tr><td colspan="8" style="text-align:center;color:var(--muted);padding:32px">No active orders.</td></tr>
      <?php endif; ?>
    </tbody>
  </table>
  </div>
...
<div class="pwf-card">
  ...
  <div style="overflow-x:auto">
  <table class="pwf-table">
    <thead><tr><th>Date</th><th>Order</th><th>Product</th><th>Craftsman</th><th>%</th><th>Note</th></tr></thead>
    <tbody>
      ...
      <?php endforeach; ?>
      <?php if (empty($logs)): ?><tr><td colspan="6" style="text-align:center;color:var(--muted);padding:24px">No logs yet.</td></tr><?php endif; ?>
    </tbody>
  </table>
  </div>
</div>
